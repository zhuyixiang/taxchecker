    ...
    public static void main(String[] args) throws DocumentException, FileNotFoundException {
        ...
        for (Order order : orders) {
            if ("FOB".equalsIgnoreCase(order.getCjType())) {
                ...
                output.append("\n");
                output.append(String.format("其中:运费:%s%s: 保费:%s%s; 货款:%s%s，折合%s；", order.getYfCurrency(), order.getYfAmount(), order.getBfCurrency(), order.getBfAmount(), order.getCurrency(), order.getGoodsAmount(), order.getGoodsAmountCNY()));
                output.append("\n");
                output.append("单位:千克\n");
                output.append("合同号：" + order.getOrderId());
                output.append("\n");
                ...
                System.err.printf("unknown CJtype: %s%n", order.getOrderNo());
            }
